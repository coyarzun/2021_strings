...
void doTriangleUp(){
  L = 0; R = width; T = 0.1*height; B = 0.9*height;
  LT = new PVector(width/2, T);
  RT = new PVector(width/2, T);
  RB = new PVector(0.8*width, B);
  LB = new PVector(0.2*width, B);
}
void doHalfHorizontal(){
  L = 0; R = width; T = height*0.33; B = 0.66*height;
  LT = new PVector(L, T);
  RT = new PVector(R, T);
  RB = new PVector(R, B);
  LB = new PVector(L, B);
...
void doTriangleDown(){
  L = 0; R = width; T = 0.1*height; B = 0.9*height;
  LT = new PVector(0.2*width, T);
  RT = new PVector(0.8*width, T);
