...
void main(void) {
    ...
    float coyarAdjust = 0.0;
    //curves
    color.r         = 0.2*(1+x1*coyarAdjust)*texture2D(textureMap, vec2(color.r, 1)).r;
    //color.r  = mod(color.r +time,1.0);
    color.g         = 0.2*(1+x2*coyarAdjust)*texture2D(textureMap, vec2(color.g, 1)).g;
    color.b         = 0.2*(1+x3*coyarAdjust)*texture2D(textureMap, vec2(color.b, 1)).b;
    ...
    for(float i=1.0; i<=1.0; ++i){
        ppp = abs(ppp*1.5)-1.0;
        float a = time*(0.25+1.0*0.1)*i;
        float c = 1.0*sin((a8*0.1+8.5)*a), s = cos((a9*0.1+0.5)*a);
        ppp*=mat2(c,s,-s,c);
    }
    
    vec2 axis = 1.0-smoothstep(10.1,1.0,abs(p));
    vec2 colors = mix(ppp,vec2(1),axis.x+axis.y);
    vec4 cc = vec4(colors*(0.85+c5), 10.5, 0.25) ;//+c5
    //////////////////////
    gl_FragColor = 1.0*cc+levels*(8.0+a7*0.1)+(1.0)*vec4(color)+(1.015+a8*0.1)*vec4(hsv2rgb(vec3(h,1.0,1.0)), 1.0);//vec4(color);// 
}